...
import psycopg2, psycopg2.extras, hashlib, os, json, csv, io, secrets
...
@login_required
def delete_post(pid):
    """Exclui uma postagem. Apenas o autor ou admin/gerente/supervisor podem excluir."""
    conn = get_db()
    c = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    if not is_owner and not is_moderator:
        c.close()
        conn.close()
        return jsonify({'error': 'Sem permissão para excluir esta postagem'}), 403

    c.execute("DELETE FROM comments WHERE post_id=%s", (pid,))
    c.execute("DELETE FROM post_likes WHERE post_id=%s", (pid,))
...
def dashboard_stats():
    ...
    try:
        ...
        try:
            c.execute("""SELECT m.type, m.quantity,
        ...
        return jsonify({
            ...
            'no_movement_products': int(no_movement),
            'critical_alerts': int(critical),
            'unread_notifications': int(notifs_unread),
            'categories': cats,
        ...
        return jsonify({'error': str(e),
                        'total_products': 0, 'low_stock': 0, 'total_kg': 0,
                        'movements_today': 0, 'no_movement_products': 0,
                        'critical_alerts': 0, 'unread_notifications': 0,
                        'categories': [], 'recent_movements': []}), 200
    finally:
        if c: c.close()
        if conn: conn.close()
...
def forgot_password():
    ...
    # (evita que alguém descubra quais e-mails estão cadastrados no sistema)
    flash('Se este e-mail estiver cadastrado, você receberá um link de recuperação em breve.')

    if not user:
        return redirect('/login')

    token = serializer.dumps(email, salt='password-reset-v2')
    reset_link = url_for('reset_password', token=token, _external=True)

    try:
        msg = Message(
            'Recuperação de Senha — START Reciclagem',
            sender=app.config['MAIL_USERNAME'],
            recipients=[email]
        )
        msg.body = f'''Olá,

Recebemos uma solicitação para redefinir sua senha no START Reciclagem.

Clique no link abaixo para criar uma nova senha:

{reset_link}

Este link expira em 1 hora. Caso não tenha solicitado, ignore este e-mail.

Equipe START Reciclagem
'''
        mail.send(msg)
    except Exception as e:
        print(f'[AVISO] Falha ao enviar e-mail de recuperação: {e}')
...
def reset_password(token):
    try:
        ...
        email = serializer.loads(token, salt='password-reset-v2', max_age=3600)
    except Exception:
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute("UPDATE users SET password=%s WHERE LOWER(email)=%s", (hash_pw(nova), email.lower()))
    conn.commit()
    c.close()
    conn.close()

    return render_template('reset_password.html', success='Senha redefinida com sucesso! Você já pode fazer login.')


# Garante que as tabelas existam ao iniciar com gunicorn (produção)
